...
const AdminStores = () => {
  ...
  // ── Data ──────────────────────────────────────────────────────────────────
  const { stores, loading, refetch } = useAdminStores({
    ...
    },
    {
      ...
      render: (row) => (
        <Button
          variant="danger"
          size="sm"
          onClick={() => setDeleteTarget(row as unknown as IStore)}
        >
          Delete
        </Button>
      ),
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        <input
          ...
          className="w-full sm:w-60 px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-primary-500"
        />
        <input
          type="search"
          value={emailFilter}
